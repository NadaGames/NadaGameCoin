...
*Setup instructions for a gitian build of NADACoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NADACoin
...
- [Building NADACoin](#building-NADACoin)
...
Clone the git repositories for NADACoin and gitian and then checkout the NADACoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/NADACoin-crypto/NADACoin.git
cd NADACoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the NADACoin repository
...
To build NADACoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the NADACoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/NADACoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/NADACoin-crypto/NADACoin
...
For example:
```bash
URL=https://github.com/crowning-/NADACoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit NADACoin=${COMMIT} --url NADACoin=${URL} ../NADACoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit NADACoin=${COMMIT} --url NADACoin=${URL} ../NADACoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit NADACoin=${COMMIT} --url NADACoin=${URL} ../NADACoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/NADACoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/NADACoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/NADACoin-build.assert
```
...
[NADACoin/gitian.sigs](https://github.com/NADACoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
